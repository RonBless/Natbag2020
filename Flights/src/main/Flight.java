...
public static boolean Menu (Scanner s, ArrayList<Flight> FlightList, Comparator<Flight> ComparFlights, File f) throws IOException {
	...
	System.out.println("1- Add a new flight\n2- Remove a flight\n3- Show all flight\n4- save to file\n5- read from\n6- see flights by monthes \n7- see flight in one month file\n8- exit");
	AllMonth(FlightList);
	String input = s.next();
	switch(input){
...
public static void AllMonth (ArrayList<Flight> FlightList) {
	for(int i=0;i<=11;i++) {
		for(int j=0;j<FlightList.size();j++) {
	if (FlightList.get(j).getMonth()==i) {
