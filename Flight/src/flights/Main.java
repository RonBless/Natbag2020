...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		Date date2 = new Date(121,2,3);
		Time time2 = new Time(3,3,0);
		FlightList.add(new Flight("El Al","Amsterdam" , "003", date2 ,  time2));
	...
	public static boolean Menu (Scanner s, ArrayList<Flight> FlightList, Comparator<Flight> ComparFlights, File f) throws IOException {
		...
		case "5":
			...
			return true;
		case "6":
			sortByDestination(s, FlightList);
			return true;
		case "7":
	...
	public static void sortByDestination(Scanner s, ArrayList <Flight> FlightList) {
		StringBuffer sb = new StringBuffer();
		System.out.println("Hello, please enter your flight destination");
		String dest = s.next();
		for(int i=0;i<FlightList.size();i++) {
			if(dest.equals(FlightList.get(i).getDestination())){
				System.out.println("The flights that are from the wanted destination are:");
